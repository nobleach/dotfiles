...
awful.keyboard.append_global_keybindings({
	...
	awful.key({ mod }, "e", function()
		awful.spawn(apps.default.nautilus)
	end, { description = "open file manager", group = "app" }),

	awful.key({ mod }, "F4", function()
		awful.spawn(apps.default.bluetooth_manager)
	end, { description = "open Blueman", group = "app" }),

	awful.key({ mod }, "F5", function()
		awful.spawn(apps.default.code)
	end, { description = "open VS code", group = "app" }),

	awful.key({ mod }, "F6", function()
		awful.spawn(apps.default.mailspring)
	end, { description = "open mailspring", group = "app" }),

	awful.key({ mod }, "F9", function()
		awful.spawn(apps.utils.hd_audio)
	end, { description = "set HD audio", group = "app" }),

	awful.key({ mod }, "F10", function()
		awful.spawn(apps.utils.headset_audio)
	end, { description = "set headeset audio", group = "app" }),


--- WM
	--- ~~
	--- Restart awesome
	awful.key({ mod, shift }, "r", awesome.restart, { description = "reload awesome", group = "WM" }),
